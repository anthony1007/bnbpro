...
//     try {
//       const [addrRes, balRes, depRes] = await Promise.all([
//         fetch("/api/user/deposit-address"),
//         fetch("/api/user/me"),
//         fetch("/api/deposit/list"),
//       ]);
//       const [addrJson, balJson, depJson] = await Promise.all([
//         addrRes.json(),
//         balRes.json(),
//         depRes.json(),
//       ]);
...
//       const newBal = balJson.balanceWei ?? "0";
//       const newDeps: Deposit[] = Array.isArray(depJson)
//         ? depJson
//         : depJson.deposits ?? [];

//       const hash = JSON.stringify([newAddr, newBal, newDeps.map((d) => d.txHash)]);
...
//     try {
//       const res = await fetch("/api/user/deposit-address", { method: "POST" });
...
//             <h2 className="text-2xl font-semibold" style={{ color: THEME.accent }}>
//               Deposit
//             </h2>
//           </div>
//         </div>
